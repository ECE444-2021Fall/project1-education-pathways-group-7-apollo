...
function MinorCoursesDisplay({userInfo, addedCourses, minorsRequirements}) {
    ...
    requirementCredits.forEach(group => {
        ...
        courseGroups.forEach(courseGroup => {
            ...
            courseGroupsForCreditRequirement = [...courseGroupsForCreditRequirement, ...coursesInCourseGroup]
        })
    ...
    return (
        <List
            ...
            subheader={<li />}
        >
        <ListSubheader style={{"fontSize": "large", "fontWeight": "bold", "textDecoration": "underline"}}>Minor Requirements</ListSubheader>
        {numRequirementCredits.map((numCreditsRequires, index) => (
          <li key={`section-${index}`}>
            <ul>
              <ListSubheader style={{"fontSize": "large", "fontWeight": "bold"}}>{`${numCreditsRequires}`} Course(s) From:</ListSubheader>
